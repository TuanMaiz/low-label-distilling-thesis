...
- Decide exactly three jointly encoding cross-encoder students: repository,
...
- Freeze teacher/provider, answer-only prompt/version, temperature, token limit,
  parser, maximum attempts, terminal reject behavior, pricing snapshot, and cost
  ceiling. No automatic fallback teacher.
...
- Freeze leakage rules: gold train labels feed only the gold arm; validation/test
  ...
  student accuracy use identical test IDs. A smaller direct cost sample is not
  an accuracy comparison.
- Audit likely benchmark contamination for teacher/students using release dates
...
Markdown is the scientific source of truth; dataset/student JSON files hold
...
- Read: `/mnt/d/study/cao-hoc/luan-van/code/models/student_config.py`
...
4. Record teacher/prompt, retry policy, direct scope, grid, seed, metrics,
...
- [ ] Plan names exact datasets/models, teacher/prompt, grid, seed, metrics,
...
source hashes. Never copy gold/entity truth into teacher prompt examples.

## Next Steps

Phase 2 implements dataset configs; Phase 4 implements student configs. Amend
