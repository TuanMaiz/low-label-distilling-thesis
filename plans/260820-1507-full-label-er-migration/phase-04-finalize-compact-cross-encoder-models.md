...
- Exactly three included compact-model configs, each pinned to an immutable
  revision with explicit `included`, backend, training, and tokenization
...
- The same model config serves gold- and LLM-supervision arms; only target
  path/source differs. Use the one planned seed; never choose reruns by outcome.
...
interface with architecture-specific adapters behind it. Resolve each source
repository to one revision, persist runtime/input-length provenance, and hash
the used config and revision into each run manifest.
...
Write failing config tests for immutable revision, cross-encoder type, pair-input
...
| Scenario | Expected |
|---|---|
| Three final configs | Load with pinned revision and eligibility metadata |
...
- [ ] Exactly three eligible, pinned compact-model configs pass smoke tests.
...
- [ ] Run-manifest inputs identify each model config/revision unambiguously.
...
Use public pinned revisions, record licenses/hashes, avoid remote code unless
audited, and validate checkpoint manifests before evaluation or reuse.
